...
(() => {
  ...
  function createExternalModules(onResolved) {
    ...
    function ensureBabel() {
      ...
      babelLoading = loadScript(babel.src, babel.integrity).catch(() => loadScript(LOCAL_BABEL_URL));
      return babelLoading;
    }
  ...
  function loadScriptWithFallback(script, fallbackSrc) {
    return loadScript(script.src, script.integrity).catch(() => loadScript(fallbackSrc));
  }
  function loadReactUmd() {
    ...
    return loadScriptWithFallback(react, LOCAL_REACT_URL).then(
      () => loadScriptWithFallback(reactDom, LOCAL_REACT_DOM_URL)
    ).then(() => void 0);
  }
  function init() {
